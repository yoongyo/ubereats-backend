...
export class RestaurantResolver {
    ...
    @Role(['Owner'])
    async deleteRestaurant(@AuthUser() authUser: User, @Args('input') deleteRestaurantInput: DeleteRestauranInput): Promise<DeleteRestaurantOutput> {
...
export class CategoryResolver {
    ...
    category(@Args('input') categoryInput: CategoryInput, page: number): Promise<CategoryOutput> {
...
export class DishResolver {
    ...
    @Role(['Owner'])
    createDish(@AuthUser() owner, @Args('input') createDishInput: CreateDishInput): Promise<CreateDishOutput> {
        return this.restaurantService.createDish(owner, createDishInput)
    }

    @Mutation(type => EditDishOutput) 
    @Role(['Owner'])
    editDish(@AuthUser() owner, @Args('input') editDishInput: EdithDishInput): Promise<EditDishOutput> {
        return this.restaurantService.editDish(owner, editDishInput)
    }

    @Mutation(type => DeleteDishOutput)
    @Role(['Owner'])
    deleteDish(@AuthUser() owner, @Args('input') deleteDishInput: DeleteDishInput): Promise<DeleteDishOutput> {
